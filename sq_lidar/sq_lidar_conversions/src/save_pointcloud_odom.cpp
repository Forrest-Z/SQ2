...
// int save_num = 20;
int save_num = 40;
int intensity_threshold = 400; // 2017/11/09 福嶋
...
void pc_callback(sensor_msgs::PointCloud2 msg){
	...
	for(size_t i=0; i<single_pc_size;i++){
		if(single_pc_->points[i].z <= z_threshold){
			if(single_pc_->points[i].intensity > intensity_threshold){
				PointType tmp;
	...
	CloudType::Ptr filtered_pc (new CloudType);
	...
	// voxel_filter.setInputCloud(output_save_pc);
	// voxel_filter.setLeafSize(0.07, 0.07, 0.07);
