...
*/
 /* グリッド範囲    70x70
  * グリッド大きさ  30cm
  * 頭きり          centerlaser_座標系から+140cm
  * 足切り          グリッド内，最下点から+30cm
  * threshold       50cm
  * intensity       あり
  */
...
HeightMap::HeightMap(ros::NodeHandle node, ros::NodeHandle priv_nh)
{
  ...
  priv_nh.param("grid_dimensions", grid_dim_, 70);
                                    ...
                                    size_t &empty_count)
{
  ...
  for (unsigned i = 0; i < npoints; ++i) {
    ...
    if (x >= 0 && x < grid_dim_ && y >= 0 && y < grid_dim_ && init[x][y]) {
	  bottom_cut = (scan->points[i].z > min[x][y] + bottom);
	  head_cut   = (scan->points[i].z < head);
      if ((max[x][y] - min[x][y] > height_diff_threshold_)    
			  && rmzero && head_cut && bottom_cut ){
