...
HeightMap::HeightMap(ros::NodeHandle node, ros::NodeHandle priv_nh)
{
  ...
  priv_nh.param("height_threshold", height_diff_threshold_, 0.20);    //0.50
                                    ...
                                    size_t &empty_count)
{
  ...
  unsigned point_num[grid_dim_][grid_dim_][remove_num] = {0};
  ...
  for (unsigned i = 0; i < npoints; ++i) {
    ...
    if (x >= 0 && x < grid_dim_ && y >= 0 && y < grid_dim_ && init[x][y]) {
	  bool bottom_cut;
	  bottom_cut=(scan->points[i].z>min[x][y]+0.30);
	  bool head_cut=(scan->points[i].z<-0.2);
      //if ((max[x][y] - min[x][y] > height_diff_threshold_)    && rmzero && head_cut &&bottom_cut ){ //&& scan->points[i].z > -0.7 ){    //change1028
      if ((max[x][y] - min[x][y] > height_diff_threshold_)    
			  && rmzero && head_cut &&bottom_cut && scan->points[i].z > -1.0 ){    //change1028
		...
		if(x>15 && x<35 && y>20 && y<30){
			if(point_num[x][y][remove_num-1] == 0){
				for(int check = 0;check<remove_num;check++){                    //change
  ...
  for (int i = 15; i < 35; i++) {
  	for (int j = 20; j < 30; j++) {
		if(point_num[i][j][remove_num-1] == 0){
