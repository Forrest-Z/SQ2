...
int main(int argc,char** argv)
{
	...
	ros::init(argc, argv, "pc2local_map_added_recovery_kmb_filter");
    ros::NodeHandle n;
	ros::Rate roop(20);
	...
	map.header.frame_id = cells.header.frame_id = "/centerlaser3_";              //frameはvelodyne
	...
	ExpandMap ex_map(0.8, 0.1);
	// ExpandMap ex_small_map(0.3, 0.1);
	ExpandMap ex_small_map(0.5, 0.1);
	...
	while (ros::ok()){
		//clock_t start=clock();
		sensor_msgs::PointCloud curv; //2016/1/24         curvatureをsub
		{
		...
		PC2GridMap(curv, map);//2016/1/24          curvatureでmap作成
