...
#define Resolution 0.05            //mapからの障害物の範囲を決定する 小さいほど障害物判定がゆるくなる（map/expandで確認）
#define recover_Resolution 0.03
...
//2016/1/24:TIS実験用にCurvatureを省いた
using namespace std;
...
int rcvry_cnt = 0;
// double radius_expand = 0.4;
sq_trajectory_generation::VelocityArray v_array;
bool v_array_flag = false;
bool tokko_go = false;

void check_recovery()
{
	if( v_array.vel.size() > 0 ){
...
void VelocityCallback(sq_trajectory_generation::VelocityArray msgs){
// void VelocityCallback(trajectory_generation::Velocity msgs){
	// linear = msgs.op_linear;
	// angular = msgs.op_linear;
	// cout<<"angular: "<<angular<<endl;
	v_array = msgs;
	v_array_flag = true;
	// cout<<"v_array_flag: true"<<endl;
	// rcvry_cnt = 40; //for debug
}
...
int main(int argc,char** argv)
{
	...
	ros::Subscriber sub_min_max = n.subscribe("sq_lidar/obstacles",1,MinMaxCallback);
	ros::Subscriber sub_v_array = n.subscribe("plan/velocity_array",10, VelocityCallback);
	ros::Publisher pub_map = n.advertise<nav_msgs::OccupancyGrid>("/local_map_real", 1);
	ros::Publisher pub_map_expand = n.advertise<nav_msgs::OccupancyGrid>("/local_map_real/expand", 1);
	...
	map.header.frame_id = cells.header.frame_id = "/centerlaser2_";              //frameはvelodyne
	...
	ExpandMap ex_map(0.9, 0.1);
	// ExpandMap ex_small_map(0.3, 0.1);
	ExpandMap ex_small_map(0.5, 0.1);       //change1015
	
	ExpandMap ex_tokko_map(0.2, 0.1);
	//tis
	...
	while (ros::ok()){
		...
		for (size_t i=0; i<MAP_SIZE; ++i)
			map.data[i]=_FREE;                //free=0
		
		PC2GridMap_minmax(minmax, map);            //minmax:障害物の点群
		pub_map.publish(map);
		if(v_array_flag) check_recovery();         //pathが引けるか判定 引けないときはexpandを小さく
		// if(!v_array_flag) recovery_level = 1; //for debug
		v_array_flag = false;
		if(recovery_level == 1){
			ex_small_map.expandObstacle(map);
			pub_map_expand.publish(ex_small_map.local_map);
		}else if(tokko_flag && recovery_level == 10){
			ex_tokko_map.expandObstacle(map);
			pub_map_expand.publish(ex_tokko_map.local_map);
		}else{
