...
bool only_real_flag = true;         //realmapのみのときはtrue
...
void real2obstacle(nav_msgs::OccupancyGrid *map){
	...
	for(int i=0; i < map->data.size(); i++){
		...
		// real_point.z = 0;
		real_point.x = (i % map->info.width);                  //点群の並び方からx.yを変換しmapに
...
int main(int argc, char** argv){
	ros::init(argc, argv, "occu_map_integrator");

	ros::NodeHandle n;

	ros::Subscriber sub1 = n.subscribe("/local_map_real/expand", 1, RealLocalMap_callback);          //点群由来
	ros::Subscriber sub2 = n.subscribe("/local_map_static/expand", 1, StaticLocalMap_callback);      //globalmap由来 pcdfile由来
	
	ros::Subscriber sub3 = n.subscribe("/arm1_start", 1, map_mode_callback);           //関係無し
