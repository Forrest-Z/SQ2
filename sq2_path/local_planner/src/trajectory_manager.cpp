//軌道生成
//trajectorymanagerが軌道生成している（ここではパラメータ調整のみ）
...
sq_trajectory_generation::TrajectoryGeneration::Response res_path;
boost::mutex req_mutex_;
sq_trajectory_generation::TrajectoryGeneration::Request req_path;
...
bool checkSelectedPath(tf::StampedTransform& transform, sq_trajectory_generation::TrajectoryGeneration& trj)       //関係なし
{
...
/////-----------Trajectory set mode------------////////
///////////////////////////////////////////////////////
void trjSetMode(sq_trajectory_generation::TrajectoryGeneration& trj, tf::StampedTransform transform)            //軌道生成のパラメータ設定
{
...
void showPath(nav_msgs::Path& path, ros::Publisher& pub)            //赤線をpub
{
...
void sq_trajectoryManage()
{
	ros::NodeHandle n;
	ros::ServiceClient client = n.serviceClient<sq_trajectory_generation::TrajectoryGeneration>("/plan/local_path");             //ここでクライアントを作る
	ros::Subscriber tiny_sub = n.subscribe("/odom", 1, tinyCallback);
	...
	ros::Publisher visu_path_pub = n.advertise<visualization_msgs::Marker>("plan/localpath_vis_shogo", 10);              //pathの赤線
	ros::Publisher vel_arr_pub = n.advertise<sq_trajectory_generation::VelocityArray>("plan/velocity_array", 10);           //pathの青線（候補）
    ...
    while(ros::ok()){
        ...
        try{
            ...
            listener.lookupTransform(header_frame, robot_frame, ros::Time(0), transform);             //mapから見たmatching base link
            tf_flag=true;
        }
        ...
        if (l_goal_flag && odometry_flag && r_flag && speed_flag && tf_flag){              //すべてのpubに成功したら
         	sq_trajectory_generation::TrajectoryGeneration trj;
            trjSetMode(trj, transform);
            if(trj.request.params.vt > 0){
                if (client.call(trj))	ROS_INFO("Changing Trajectory");                  //ここでtrajectorygenerationを呼び出す
...
{
	ros::init(argc, argv, "sq_trajectory_manager");
	sq_trajectoryManage();
	return 0;
}
