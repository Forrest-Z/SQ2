...
const float Vmin = 0.30;        //0920
const float Vmax = 1.00;
const float Lmin = 2.5;//previous 3.0
const float Lmax = 6.0;  //5.0  localpathの長さを変える
...
float speedDecision()                    //関係なし
{
	...
	switch (wp_mode){
		case SLOW:
			...
			// speed_ratio.data=0.5;             //change origin 0.7
			speed_ratio.data=0.6;             //change origin 0.7
			break;
		case NORMAL:
			// speed_ratio.data=0.5;
			speed_ratio.data=0.6;             //change origin 0.7
			break;
		case FAST:
			// speed_ratio.data=0.5;
			speed_ratio.data=0.6;
			break;
		default:
			// speed_ratio.data=0.5;
			speed_ratio.data=0.6;
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber wp_mode_sub = n.subscribe("/speed_mode", 1, WpModeCallback);///関係なし
