...
{
	return hypot(fabs(A.x-B.x), fabs(A.y-B.y));              //平方和の平方根
}
...
void findLocalGoal(geometry_msgs::PoseStamped& goal, geometry_msgs::PoseStamped& robo, ros::Publisher& pt_on_line_pub)
{
	...
	A.x = p_wp.pose.position.x;          //previouswaypoint
	A.y = p_wp.pose.position.y;
	B.x = t_wp.pose.position.x;          //目指すwaypoint
...
inline bool checkTF(tf::StampedTransform& transform, geometry_msgs::PoseStamped& robo, tf::TransformListener& listener)             //mapframeでのrobotの座標をだす
{
	try{
		//listener.waitForTransform(header_frame, robot_frame, ros::Time(0), ros::Duration(1.0));
	    listener.lookupTransform(header_frame, robot_frame, ros::Time(0), transform);         //mapからmatchingbaselinkを見た座標
...
{
	t_wp=*msg;
	callback_flag[0]=true;                         //目指しているwaypointが受信できたらtrue
}

void PreviousWpCallback(const geometry_msgs::PoseStampedConstPtr& msg)
{
	p_wp=*msg;
	callback_flag[1]=true;                         //直前に通過したwaypointを受信したらtrue
}
...
{
	n_dist=msg->data;
	callback_flag[2]=true;                         //waypoint間の直線との距離を受信したらtrue
}
...
{
	ros::NodeHandle n;
	ros::Subscriber twp_sub = n.subscribe("/target/pose", 1, TargetWpCallback);                 //現在目指しているwaypoint
	ros::Subscriber pre_sub = n.subscribe("/waypoint/prev", 1, PreviousWpCallback);             //直前に通過したwaypoint
	ros::Subscriber norm_dist_sub = n.subscribe("/waypoint/normal_dist", 1, NormalDistCallback); //waypoint間の直線との距離
	ros::Subscriber radius_sub = n.subscribe("/control/r", 1, LocalPathRadiusCallback);          //speedcontrolからpub
	ros::Subscriber avoid_flag_sub = n.subscribe("/avoid_flag", 1, AvoidFlagCallback);          //関係なし
	...
	while(ros::ok()){
		//get robot position
		bool tf_flag = checkTF(transform, robo, listener);           //roboの座標算出
