...
int main(int argc, char** argv)
{
	...
	ros::Subscriber sub = n.subscribe("/local_map_real/expand", 1, map_callback);
    ...
    while (ros::ok()) {
		...
		try{
			...
			listener.lookupTransform(header_frame, sensor_frame, ros::Time(0), transform);          //map座標系からvelodyne座標系へ変換
            tf_flag=true;
		}
		...
		if(callback_flag && tf_flag){
			...
			map_pub.publish(grid_map);                //map座標系で移動したlocalmapを見ることができる
            //callback_flag=false;
		}
