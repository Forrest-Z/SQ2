...
#include "sq_trajectory_generation/trajectory_generation.h"
#include "sq_trajectory_generation/boundary_state.h"
#include "sq_trajectory_generation/LocalPlanning_lib.h"
...
const float Vmax = 0.6;	    //0920
const float Vmax_ = 0.6;
const float Lmin = 2.0;
...
typedef struct {
	...
	nav_msgs::Path path;
	sq_trajectory_generation::VelocityArray v_a;
	u_int id;
...
// ShapeParameter p_ss = {1000, 31, 5, 8.0, -1.6, 1.6, -0.9, 0.9};	//20170920
...
void showPathArray(list<PathState> path_array, int num)         //visualise
{
	std_msgs::ColorRGBA rgba_in;
	rgba_in.r = 0.0;
	rgba_in.g = 0.8;
...
/////--------------Target maker----------------////////
///////////////////////////////////////////////////////
void targetMaker(sq_trajectory_generation::TrajectoryGeneration::Request req, 
...
void knmConvertor(sq_trajectory_generation::TrajectoryGeneration::Request req,
					geometry_msgs::PoseStamped& x_f)
{
	//p_ss.d = req.r;
	//float theta = req.goal.pose.position.x;
...
///////////////////////////////////////////////////////
float setBoundaryStates(sq_trajectory_generation::TrajectoryGeneration::Request req,           //軌道生成のための設定
						geometry_msgs::PoseArray& boundary_state)
{
	...
	return_n = (p_ss.n_p * p_ss.n_h);         //目的地数x目的地あたりの本数
	//p_ss.d = req.r;
...
void generatePathArray(sq_trajectory_generation::TrajectoryGeneration::Request req,
						geometry_msgs::PoseArray boundary_state, 
						list<PathState>& path_array)
{
	...
	if (callback_flag){                      //mapが受信できていたら
		for(u_int i=0; i<boundary_state.poses.size(); i++){
			...
			sq_trajectory_generation::TrajectoryGeneration::Response path_v;
			...
			path_state.cost = pathCheck(path_state.path, gmap);            //localplanning libへ
...
bool server(sq_trajectory_generation::TrajectoryGeneration::Request  &req,           //ここがtrajectorygeneration
         sq_trajectory_generation::TrajectoryGeneration::Response &res )
{
	clock_t start,end;
	start = clock();
	
//--------Teledrive Mode !!
	if(req.teledrive){              //基本false
		geometry_msgs::PoseStamped x_f;
		targetMaker(req, x_f);
		showPose(x_f);
		req.goal = x_f;
    ...
    // cout<<"bbb"<<endl;
	geometry_msgs::PoseArray boundary_state;                 //xyzw等
...
int main(int argc, char **argv)
{
	...
	ros::ServiceServer service = n.advertiseService("/plan/local_path", server);                      //trajectorygenerationの定義
	ros::Subscriber sub1 = n.subscribe("/local_map/expand/gc", 5, mapCallback);       //0921
	//ros::Subscriber sub1 = n.subscribe("/local_map_real/expand", 5, mapCallback);
